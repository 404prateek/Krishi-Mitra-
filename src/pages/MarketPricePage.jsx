...
const MarketPricePage = () => {
  ...
  // enabled=false → no fetch on mount; becomes true when user interacts
  const [enabled, setEnabled] = useState(false);

  const { prices, loading, error, lastUpdated, source, refetch } =
    useMarketPrices(selectedState, commodity, enabled);
        ...
        <div className="market-controls">
          <div className="controls-grid">
            ...
            >
              ...
              <span className="btn-text">{loading ? "Loading…" : enabled ? "Refresh" : "Get Prices"}</span>
            </button>
          </div>
        </div>

        {/* Results area — idle / loading / error / data */}
        {!enabled && !loading && (
          <div className="market-idle-state">
            <div className="idle-icon">🌾</div>
            <h3>Select a state &amp; commodity</h3>
            <p>Choose filters above or click <strong>Get Prices</strong> to view live mandi rates.</p>
          </div>
        )}

        {loading && (
          <div className="market-results-loader">
        ...
        {!loading && !error && enabled && (
          <>
            ...
            <div className="results-summary">
              ...
              {lastUpdated && (
                <p style={{ marginTop: "2px", fontSize: "12px", color: "var(--text-secondary)" }}>
            ...
            <div className="price-cards">
              {prices.length === 0 ? (
                <div className="no-results">
                  ...
                  <p>Try adjusting your filters or check back later for updated prices.</p>
                </div>
              ) : (
                  ...
                  return (
                    <div key={index} className="price-card">
                      ...
                      <div className="card-footer">
                        <span className="update-time">Arrival Date: {item.date}</span>
                      </div>
                    </div>
                  );
                })
            ...
            {prices.length > 0 && (
              <div className="market-insights">
                ...
                <div className="insights-grid">
                  ...
                  <div className="insight-card">
                    <h4>Best Markets</h4>
                    <p>Compare prices across different regions for better profits</p>
                  </div>
                </div>
              </div>
            )}
